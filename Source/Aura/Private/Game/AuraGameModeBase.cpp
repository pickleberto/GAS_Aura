...
ULoadScreenSaveGame* AAuraGameModeBase::GetSaveSlotData(const FString& SlotName, int32 SlotIndex) const
{
	...
	ULoadScreenSaveGame* AuraSaveData = Cast<ULoadScreenSaveGame>(SaveGameObject);
	AuraSaveData->SlotName = SlotName;
	AuraSaveData->SlotIndex = SlotIndex;
...
void AAuraGameModeBase::SaveInGameProgressData(ULoadScreenSaveGame* SaveObject)
{
	...
	const FString SaveSlotName = AuraGameInstance->LoadSlotName;
	const int32 SaveSlotIndex = AuraGameInstance->LoadSlotIndex;
...
void AAuraGameModeBase::LoadWorldState(UWorld* World) const
{
	FString WorldName = World->GetMapName();
	WorldName.RemoveFromStart(World->StreamingLevelsPrefix);
	UAuraGameInstance* AuraGI = CastChecked<UAuraGameInstance>(GetGameInstance());
		...
		for (FActorIterator It(World); It; ++It)
		{
			...
			for (FSavedActor SavedActor : SaveGame->GetSavedMapWithMapName(WorldName).SavedActors)
			{
				if (SavedActor.ActorName == Actor->GetFName())
				{
					...
					// now we will READ data from Bytes
