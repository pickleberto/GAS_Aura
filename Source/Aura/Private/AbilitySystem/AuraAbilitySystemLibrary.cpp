...
void UAuraAbilitySystemLibrary::GetClosestTargets(int32 MaxTargets, const TArray<AActor*>& Actors, TArray<AActor*>& OutClosestsTargets, const FVector& Origin)
{
	if (Actors.Num() <= MaxTargets)
	{
		OutClosestsTargets = Actors;
		return;
	}
	
	TArray<AActor*> Targets = Actors;
	while (OutClosestsTargets.Num() < MaxTargets)
	{
		if (Targets.Num() == 0) break;
		...
		Targets.Remove(ClosestActor);
		OutClosestsTargets.AddUnique(ClosestActor);
	}
}
...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageParams)
{
	...
	SetIsRadialDamage(Context, DamageParams.bIsRadialDamage);
	SetRadialDamageInnerRadius(Context, DamageParams.RadialDamageInnerRadius);
	SetRadialDamageOuterRadius(Context, DamageParams.RadialDamageOuterRadius);
	SetRadialDamageOrigin(Context, DamageParams.RadialDamageOrigin);
...
void UAuraAbilitySystemLibrary::SetKnockbackDirection(FDamageEffectParams& DamageEffectParams, FVector Direction, float Magnitude)
{
	const float ForceMagnitude = (Magnitude == 0) ? DamageEffectParams.KnockbackForceMagnitude : Magnitude;
	Direction.Normalize();
	DamageEffectParams.KnockbackForce = Direction * ForceMagnitude;
}

void UAuraAbilitySystemLibrary::SetDeathImpulseDirection(FDamageEffectParams& DamageEffectParams, FVector Direction, float Magnitude)
{
	const float ForceMagnitude = (Magnitude == 0) ? DamageEffectParams.DeathImpulseMagnitude : Magnitude;
	Direction.Normalize();
	DamageEffectParams.DeathImpulse = Direction * ForceMagnitude;
}

void UAuraAbilitySystemLibrary::SetRadialDamageEffectParams(FDamageEffectParams& DamageEffectParams, bool bIsRadial, float InnerRadius, float OuterRadius, FVector Origin)
{
...
void UAuraAbilitySystemLibrary::SetIsRadialDamage(FGameplayEffectContextHandle& EffectContextHandle, bool bValue)
{
	if (FAuraGameplayEffectContext* AuraContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraContext->SetIsRadialDamage(bValue);
	}
}

void UAuraAbilitySystemLibrary::SetRadialDamageInnerRadius(FGameplayEffectContextHandle& EffectContextHandle, float Value)
{
	if (FAuraGameplayEffectContext* AuraContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraContext->SetRadialDamageInnerRadius(Value);
	}
}

void UAuraAbilitySystemLibrary::SetRadialDamageOuterRadius(FGameplayEffectContextHandle& EffectContextHandle, float Value)
{
	if (FAuraGameplayEffectContext* AuraContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraContext->SetRadialDamageOuterRadius(Value);
	}
}

void UAuraAbilitySystemLibrary::SetRadialDamageOrigin(FGameplayEffectContextHandle& EffectContextHandle, const FVector& InValue)
{
	if (FAuraGameplayEffectContext* AuraContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraContext->SetRadialDamageOrigin(InValue);
	}
}
